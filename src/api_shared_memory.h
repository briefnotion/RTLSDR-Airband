  ...
  // Will not send if Destination has not ACKed ready.
  {
    if (PAUSE == false)
    {
      ...
      // Prevent data being read if the packet hasn't been completely written.
      {
        ...
        {
          (*SQUELCH).COMMAND_FROM_RADIO.COMMAND = Command_Send.COMMAND;
          (*SQUELCH).COMMAND_FROM_RADIO.PARAMETER = Command_Send.PARAMETER;

          (*SQUELCH).COMMAND_FROM_RADIO.CHANGED = true;
          Command_Send.CHANGED = false;
        }

        // Receive Command
        if ((*SQUELCH).COMMAND_TO_RADIO.CHANGED == true)
        {
        Command_Recv.COMMAND = (*SQUELCH).COMMAND_TO_RADIO.COMMAND;
        Command_Recv.PARAMETER = (*SQUELCH).COMMAND_TO_RADIO.PARAMETER;

        Command_Recv.CHANGED = true;
        (*SQUELCH).COMMAND_TO_RADIO.CHANGED = false;
        }
